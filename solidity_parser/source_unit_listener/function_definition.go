...
func (su *FunctionDefinition) GetId() string {
	return su.Id
}

func (su *FunctionDefinition) SetDescription(description string) {
	su.Description = description
}

func (su *FunctionDefinition) UpdateDescription(id string, description string) {
	if su.Id == id {
	...
	for i := range su.Returns {
		su.Returns[i].UpdateDescription(id, description)
