...
func (su *ContractDefinition) GetId() string {
	return su.Id
}

func (su *ContractDefinition) SetDescription(description string) {
	su.Description = description
}

func (su *ContractDefinition) UpdateDescription(id string, description string) {
	if su.Id == id {
		su.SetDescription(description)

	}

	for i := range su.Variables {
		su.Variables[i].UpdateDescription(id, description)
	...
	for i := range su.Modifiers {
		su.Modifiers[i].UpdateDescription(id, description)
